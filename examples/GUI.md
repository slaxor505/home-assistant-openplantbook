...
Create these helpers to store search results and trigger the automations. You can create them via YAML or in the UI
...
A Markdown card that shows the selected plant's image, species, thresholds, and (when fetched with `include: care`) care
guidance:
